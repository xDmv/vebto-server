...
class Oauth {
    ...
    {
        if (Auth::user()) {
           return View::make('vebto::oauth/popup')->with('status', 'ALREADY_LOGGED_IN');
        }
    ...
    {
        return response()->view('vebto::oauth.popup', ['status' => 'ERROR', 'data' => $message], 500);
    }
    ...
    {
        $view = View::make('vebto::oauth/popup')->with('status', $status);
